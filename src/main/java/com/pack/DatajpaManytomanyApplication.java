...
public class DatajpaManytomanyApplication   implements  CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		 ...
		 * 
		 * Optional<Book> opt=bookRepo.findById(2); Book book=opt.get(); Set<Author>
		 * authors=book.getAuthors();
		 * 
		 * 
		 * Optional<Author> opt2 = authorRepo.findById(103); Author author=opt2.get();
		 * 
		...
		//add a new author  to a book with id 1
		  ...
		  Author new_author=new Author(104, "D"); Optional<Book>
		  opt=bookRepo.findById(1); Book book = opt.get(); Set<Author>
		  authors=book.getAuthors();
